        ...
        <h3>${weather[0].main}</h3>
            ...
            <div class="current-weather-right">
              ...
              <div class="current-weather-description underline-bottom">
                ...
                <div><p>${Math.round(feels_like)}°<sup>c</sup></p></div>
              </div>
              <div class="current-weather-description">
                <div><p class='description-text'>Wind</p></div>
                <div><p>${Math.round(wind_speed)} KPH</p></div>
              </div>
            </div>
          </div>
    `;
...
const generateCurrentCity = (currentCity, data) => {
  ...
  return `
    <h2>${currentCity} &nbsp;${Math.round(temp)}°c</h2>
    <img class="city-icon" style="height: 60px" src="${icon}" alt="${
    weather[0]["description"]
  }">
  `;
};
  ...
  for (let item of data.daily) {
    ...
    const markup = `
          ...
          <div class="forecast-info-container">
            ...
            <img class="forecast-img" src="${icon}" alt="${
      weather[0]["description"]
    }">
            </div>
            <div class="forecast-temp">
              <h1>${Math.round(temp.max)}<sup>°</sup>c</h1>
            </div>
